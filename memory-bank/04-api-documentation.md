...
| Endpoint                                          | Método | Auth                   | Notas                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------- | ------ | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/api/consultations/:id/share`                    | POST   | AppAuthGuard (doctor)  | Body: `{ sections: { report, prescriptions, ehr } }` (al menos 1 true). **Guard: si el paciente no tiene cédula → 422 `PATIENT_CEDULA_REQUIRED_FOR_SHARING`** (la cédula es 2º factor en verify-code; sin ella el enlace es inusable). Respuesta envuelta `{ success, data:{ url, code, expiresAt } }`. `url` usa `APP_BASE_URL` del backend (ya no `localhost`). Email fire-and-forget. Token = 48 bytes base64url. |
