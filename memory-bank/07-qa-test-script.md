...
## D-2026-07-16) Lote grande "Prueba 15-07.txt" (~27 obs) + WhatsApp recordatorio

> Probar en `delta-frontend-knliodnwza-ue.a.run.app` (NO deltasalud.app). Doctor de prueba: para IA usar un
> doctor **delta_plus**; para el resto sirve cualquier plan. Regla: un caso PASA solo si el efecto se ve en la
> pantalla resultante.

| ID      | Módulo / pantalla                              | Pasos                                                                                                    | Resultado esperado                                                                                                                                         | Verificación BD / nota                                                            |
| ------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| D-16.1  | Nuevo paciente (Inicio/Pacientes/Consulta)     | Crear 2 pacientes con el **mismo email**                                                                 | Ambos se crean (email duplicado permitido)                                                                                                                 | 2 filas en `patients` con mismo `email_search_hash`                               |
| D-16.2  | Nuevo paciente                                 | Usar como email el **del propio doctor** (especialista)                                                  | Error 409: "No puedes usar tu propio correo…"                                                                                                              | `PatientEmailIsDoctorError`; no se crea                                           |
| D-16.3  | Nuevo paciente                                 | Cédula de **4 dígitos** y de **15 dígitos**                                                              | Ambas válidas (antes exigía 6-10)                                                                                                                          | `CedulaInput` 4-15; DTO `min(4)`                                                  |
| D-16.4  | Nuevo paciente                                 | Dejar email Y teléfono vacíos → Guardar                                                                  | Bloquea: "Indica al menos un correo o un teléfono"                                                                                                         | validación front + `.refine` DTO                                                  |
| D-16.5  | Modal Nueva consulta / cita (tipo de consulta) | Abrir el selector de tipo de consulta                                                                    | **NO** aparece la duración en minutos                                                                                                                      | `StepServiceType` (y booking)                                                     |
| D-16.6  | Registrar ingreso (Inicio y Consultas)         | Marcar "Asociar a una consulta" → Guardar                                                                | Sin error `Unrecognized key`; el ingreso queda asociado                                                                                                    | body manda `related_consultation_id`; `financial_transactions`                    |
| D-16.7  | Consulta — panel de pago                       | Marcar pagada/Aprobar **sin** método de pago                                                             | Abre **modal de método de pago** (método obligatorio, referencia+comprobante opcionales); al confirmar se marca pagado                                     | `PATCH :id/payment-details` + aprobar; `consultations.payment_method`             |
| D-16.8  | Pacientes — lista                              | Ver el buscador                                                                                          | **NO** hay filtro por canal a la derecha                                                                                                                   | select eliminado                                                                  |
| D-16.9  | Ficha paciente → Historial médico (delta_plus) | Botón del banner IA                                                                                      | Botón **"Generar resumen"** → llama IA y muestra el resumen (NO lleva a planes)                                                                            | `/api/doctor/ai` action `patient_history`; gating `ai_assistant`                  |
| D-16.10 | Ficha paciente → Historial médico (delta_free) | Botón del banner IA                                                                                      | Botón **"Ver planes"** → `/doctor/upgrade`                                                                                                                 | feature `ai_assistant` off                                                        |
| D-16.11 | Ficha paciente → Seguimiento                   | Abrir la pestaña                                                                                         | **NO** aparece "Este paciente aún no tiene cuenta"; **NO** están "Pedirle algo al paciente" ni "Historial compartido"; SÍ está "Solicitudes de documentos" | `SHOW_SHARED_FILES_CARDS=false`                                                   |
| D-16.12 | Solicitudes / ficha — adjunto del paciente     | Descargar un archivo que subió el paciente                                                               | Abre en **pestaña nueva**                                                                                                                                  | `RequestDetailModal` `target=_blank`                                              |
| D-16.13 | Consulta — Generar documento → Informe         | Marcar "Informe": listar bloques a incluir                                                               | Aparecen **récipe y paraclínico** (además de motivo/diagnóstico/etc.) y se pueden marcar                                                                   | `buildPdfContent` inyecta bloques estructurados                                   |
| D-16.14 | Consulta — Compartir documentos                | Seleccionar Informe + **todos** los bloques → generar enlace → abrir el PDF del paciente                 | El PDF trae **todos** los bloques seleccionados (no solo "motivo")                                                                                         | ruta `[token]/pdf` inyecta prescripciones; paraclínico requiere consulta guardada |
| D-16.15 | Plantillas → Recibo                            | Abrir la tab Recibo                                                                                      | **NO** aparece el aviso "Configuración del recibo guardada localmente"; la vista previa **NO** se refresca en bucle                                        | `TemplatePdfPreview` memoiza `docDate`                                            |
| D-16.16 | Cobros / consulta / WhatsApp                   | Con tasa **personalizada** o **EUR** en Métodos de pago, ver el costo en Bs de una consulta              | El Bs se calcula con la **misma** tasa seleccionada en Métodos de pago                                                                                     | `useBcvRate` por modo desde `/api/admin/bcv-rate` (usd/eur) o custom              |
| D-16.17 | Recordatorio — WhatsApp                        | Enviar recordatorio por WhatsApp a un paciente                                                           | Emojis **se ven bien** (👋📅🕐🔖🏥); **NO** aparece "Si necesitas reagendar, contáctanos con anticipación."                                                | `buildWAMessage` con `\u{}`                                                       |
| D-16.18 | Email — solicitud de documentos                | Enviar solicitud al paciente y abrir el correo                                                           | El botón "Responder solicitud" tiene **letras blancas**                                                                                                    | mig `20260716000001` `color:#ffffff` inline                                       |
| D-16.19 | Booking público `/book/:id`                    | Abrir en una fecha con horarios ya tomados y con bloqueos del doctor                                     | Los horarios **ocupados y bloqueados** se ven deshabilitados; si el slot se toma al confirmar, el error sale **junto a la grilla**                         | endpoint `available:false`; render corregido                                      |
| D-16.20 | Imágenes del doctor                            | Ver avatar/logo del doctor (settings, plantillas PDF) y la imagen en el booking público tras varios días | Las imágenes **se ven** (no rotas por signed URL expirada)                                                                                                 | `resignGcsImageUrl` re-firma al leer                                              |

---

## D-2026-07-17) 2ª tanda de QA (10 obs) — bugs que el tester superficial dejó pasar

> **Regla reforzada:** un caso PASA sólo si se reproduce el efecto real (HTTP 200 / fila en BD / DOM), NO por
> "no dio error". Los 2 de mayor impacto (slots, prescripciones) eran **silenciosos** (404 que devuelve HTML;
> 400 tragado en un toast). Para probar Cobros/Finanzas/IA: flip de `lucas@deltasalud.app` a `delta_plus`+`doctor`
> (revertir a super_admin/NULL al terminar). Prod: `delta-frontend-knliodnwza-ue.a.run.app`.

| ID      | Módulo / pantalla                       | Pasos                                                                                         | Resultado esperado                                                                                                | Verificación real                                                                                                    |
| ------- | --------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| D-17.1  | Link público `/book/:id`                | Abrir una fecha con horarios ya tomados / con bloqueos del doctor                             | Los horarios **ocupados y bloqueados** salen deshabilitados; error del slot **junto a la grilla**                 | `GET /api/booking/:id/slots?date=` devuelve JSON `{slots:[{time,available}]}` con `available:false` (antes 404 HTML) |
| D-17.2  | Consulta → bloque Paraclínico           | Agregar examen (sin presentación) → **Guardar**                                               | Guarda sin "Algunas prescripciones fallaron"; toast de éxito                                                      | `POST /api/doctor/prescriptions` → **200** (antes 400 "expected string, received null")                              |
| D-17.3  | Consulta → Compartir documentos         | Con récipe guardado, compartir Informe+Récipe → descargar el PDF del paciente (cédula+código) | El PDF trae **informe + récipe + indicaciones** (no solo motivo)                                                  | El PDF descargado tiene **3 páginas** (antes 1); `prescriptions` del render-data no vacío                            |
| D-17.4  | Consulta pagada → "Ingreso adicional"   | Registrar un ingreso asociado a la consulta → Guardar                                         | "Ingreso registrado"; aparece en Finanzas                                                                         | `financial_transactions` +1 con `related_consultation_id` seteado (desde el inicio ya guardaba)                      |
| D-17.5  | Consulta → marcar pagado SIN método     | Estado del pago → "Aprobado" sin método → modal → elegir método → Confirmar                   | La consulta queda **PAGADA** con el método, en **un solo modal** (no abre un 2º modal ni queda pendiente)         | `consultations.payment_status='approved'` + `payment_method` seteado                                                 |
| D-17.6  | **Inicio** → "Cobros" (Registrar pago)  | Aprobar un cobro pendiente **sin** método → modal → elegir método → Confirmar                 | Abre el modal de método (no da error de backend) y aprueba                                                        | `payments.status='approved'` + `method_snapshot` seteado                                                             |
| D-17.7  | Cobros → detalle de un cobro ya pagado  | Abrir el detalle de un pago aprobado con monto Bs guardado                                    | La **tasa mostrada es la congelada** del cobro (Bs÷USD), no la del día                                            | `editBcvRate` = `amount_bs/plan_price` si hay `amount_bs`                                                            |
| D-17.8  | Plantillas → Recibo                     | Abrir la pestaña Recibo                                                                       | La vista previa es el **recibo real** (mismo de Cobros), no un documento react-pdf; sin loop de refresco          | Renderiza `buildReceiptHtml` en iframe (`ReceiptPreview`)                                                            |
| D-17.9  | Recordatorios → vista previa / WhatsApp | Ver la vista previa del mensaje / enviar por WhatsApp                                         | Emojis correctos (👋📅🕐📋); **sin** "Si necesitas reagendar…"                                                    | `buildWAMessage` con escapes `\u`                                                                                    |
| D-17.10 | Landing pública `/` (o `/landing.html`) | Bajar a la sección de precios                                                                 | **3 tarjetas**: Delta Free (Gratis), Delta Base ($10, Recomendado), Delta Plus ($30), con features + CTA por plan | `GET /api/public/plans` devuelve los 3 planes desde BD                                                               |
| D-17.11 | Consulta → bloque Paraclínico           | Ver los botones del bloque                                                                    | Ya **no** existe el botón "PDF" viejo; el PDF sale por "Generar Documento" (branded)                              | (cosmético)                                                                                                          |

---

## D-2026-07-18) 3ª+4ª tanda QA — guardado de bloques (CRÍTICO), firma PDF, teléfono intl, lote de 9

> Verificados EN VIVO con Playwright. Muchos bugs reportados por el usuario eran **bundle viejo en la pestaña**
> (SPA no recarga el JS al navegar): la regla de QA es `Cmd+Shift+R` / pestaña nueva antes de reportar.

| Caso    | Módulo / acción                                    | Pasos                                                                                                | Esperado                                                                    | Verificación BD / técnica                                                                                                                                                         |
| ------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-18.1  | **Consulta → Paraclínico (PÉRDIDA DE DATOS)**      | Agregar exámenes en Paraclínico + Guardar → salir → reabrir                                          | Los exámenes **siguen** ahí (antes se borraban)                             | `blocks_snapshot->'paraclinical'` = array `["examen — notas"]` (NO en tabla prescriptions)                                                                                        |
| D-18.2  | **Compartir Paraclínico**                          | Llenar paraclínico → Compartir documentos → tildar solo "Paraclínicos" → generar                     | 200 con enlace+código; el PDF trae "EXÁMENES PARACLÍNICOS" con los exámenes | POST `/share` 200; PDF `pdftotext` contiene los exámenes                                                                                                                          |
| D-18.3  | **Autosave reposo NO borra bloques**               | Guardar antecedentes/examen físico/paraclínico → abrir una consulta con diagnóstico → recargar       | Todos los bloques **sobreviven** (antes el autosave del reposo los borraba) | `blocks_snapshot` conserva todas las keys                                                                                                                                         |
| D-18.4  | **Escenario combinado (agregar bloque + valores)** | Llenar paraclínico + antecedentes + **agregar bloque nuevo** ("Ejercicios") con contenido → recargar | Los 3 sobreviven; el bloque nuevo aparece con su pestaña y valor            | `blocks_snapshot` tiene paraclinical+history+exercises; `blocks_structure` incluye `exercises`. Estructura y valores = columnas separadas (update parcial `!== undefined → skip`) |
| D-18.5  | **Renombrar bloque no pierde valor**               | Renombrar un bloque de texto (config)                                                                | El valor persiste (indexado por KEY, no label)                              | `blocks_snapshot[key]` intacto tras cambiar label                                                                                                                                 |
| D-18.6  | Consulta → "Mejorar redacción" IA                  | Ver el picker de bloques a mejorar                                                                   | Solo bloques de TEXTO LIBRE; **sin** Paraclínico/Récipe/Reposo              | (cosmético/UX)                                                                                                                                                                    |
| D-18.7  | Consulta → Referencia + IA                         | Escribir en Referencia → Mejorar redacción → Aplicar → recargar                                      | El texto mejorado **persiste**                                              | `blocks_snapshot->'requested_exams'` = string (no array)                                                                                                                          |
| D-18.8  | Finanzas → borrar gasto                            | Crear gasto → Gastos → botón borrar (confirmar) → recargar                                           | El gasto **desaparece** y no reaparece                                      | `DELETE /api/finances/transactions/:id`; fila borrada en `financial_transactions`                                                                                                 |
| D-18.9  | Finanzas → editar/borrar ingreso                   | Ingresos manuales → botones editar/borrar → recargar                                                 | Persiste el cambio/borrado                                                  | idem transactions (income)                                                                                                                                                        |
| D-18.10 | Inicio → Registrar ingreso                         | Abrir modal                                                                                          | Tiene botón **"Crear paciente"** (como finanzas)                            | —                                                                                                                                                                                 |
| D-18.11 | Inicio → Citas del día                             | Ver el cuadro con >3 citas                                                                           | Muestra **todas** (scroll), no solo 3                                       | —                                                                                                                                                                                 |
| D-18.12 | Config → Firma dibujada                            | Firma → "Dibujar" → dibujar en canvas → Guardar firma                                                | Sube PNG y actualiza preview                                                | `POST /api/storage/upload` kind=signature → `profiles.signature_url`                                                                                                              |
| D-18.13 | Firma/logo en **PDF compartido**                   | Compartir con firma configurada → descargar PDF                                                      | El PDF trae firma + logo                                                    | `pdfimages` muestra las 2 imágenes; backend re-firma URLs GCS vencidas (`resignGcsImageUrl`)                                                                                      |
| D-18.14 | Recordatorio WhatsApp — teléfono intl              | Paciente con teléfono +56 (u otro país) → Recordatorios → WhatsApp                                   | Genera el enlace `wa.me` (antes "Teléfono inválido")                        | `normalizePhoneIntl` acepta E.164 8-15 díg                                                                                                                                        |
| D-18.15 | Recordatorio WhatsApp — sin emojis                 | Recordatorios → WhatsApp                                                                             | Mensaje con **guiones** (`-`), sin `�`                                      | `buildWAMessage` sin emojis (la pantalla intermedia de WA rompe cualquier emoji)                                                                                                  |
| D-18.16 | Landing → recomendado                              | Bajar a precios                                                                                      | Badge "Recomendado" en **Delta Plus** (mayor precio)                        | tarjetas + tabla comparativa                                                                                                                                                      |

**Regla de arquitectura (guardado de bloques):** el backend REEMPLAZA todo `blocks_snapshot` en cada PATCH → **todo
guardado de `blocks_data` DEBE mergear el snapshot COMPLETO** del ref (nunca un bloque suelto). Columnas separadas:
`blocks_snapshot` (valores por key) vs `blocks_structure` (estructura/labels), con update parcial independiente.
