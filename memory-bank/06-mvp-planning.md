...
| 7    | IA con Gemini (transcripción + texto: mejorar/resumir/historial)                          | bloqueado  | Código completo y desplegado (texto `/api/ai/text` commit `b25522b` + transcripción). Gating por plan OK (IA solo en Plus). 🚨 BLOQUEADO por Google: la key de Gemini da 403 "project denied access" → toda IA da 502. Falta que el usuario arregle el acceso a Gemini. |
