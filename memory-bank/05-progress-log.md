...
## 2026-07-23 — "Consultas por agendar" (preconsultas) + terminología especialista ✅ (implementado+revisado, PENDIENTE QA staging)

Sesión de equipo de agentes (lead + backend-agent + frontend-agent + security/code review). NO commiteado aún → va por `feature/* → develop → staging → master`. tsc/eslint/jest verdes en todo; boot dist + curl pendientes para la ventana de QA (regla no-Docker).
