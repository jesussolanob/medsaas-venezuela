...
export default function PatientSeguimientoPage() {
  ...
  const [uploadModal, setUploadModal] = useState<{ open: boolean; replyTo?: SharedFile | null }>({
    open: false,
  });
  ...
  const pendingTasks = visibleFiles.filter(
    (f) => f.category === 'instruction' && f.status === 'pending' && f.created_by === 'doctor',
  );
  ...
  const completedFiles = visibleFiles.filter(
    (f) =>
      f.file_url ||
      f.category === 'comment' ||
  ...
  function resolveTargetDoctor(
    replyTo: SharedFile | null | undefined,
  ): { doctorId: string; patientId: string } | null {
    if (replyTo) {
    ...
    try {
      ...
      if (file) {
        ...
        const { error } = await replyWithComment(supabase, {
          ...
          title:
            uploadTitle.trim() ||
            (uploadModal.replyTo ? `Re: ${uploadModal.replyTo.title}` : 'Comentario'),
    ...
    return (
      <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
        ...
        <p className="text-sm text-slate-400 mt-1">
          Tu seguimiento aparecerá aquí cuando agendes con un doctor.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-5 sm:space-y-6">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-2">
                      {doctors.length > 1 && (
                        <span className="font-semibold">{getDoctorName(task.doctor_id)} · </span>
                      )}
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {visibleLegacyRx.map((rx) => {
              const meds = (Array.isArray(rx.medications) ? rx.medications : []).filter(
                (m) => m.name,
              );
                ...
                <div key={rx.id} className="p-4 sm:p-5">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs text-slate-500">
                      {doctors.length > 1 && (
                        <span className="font-semibold">{getDoctorName(rx.doctor_id)} · </span>
                      )}
      ...
      {editingFile && (
        <div
          ...
          onClick={() => !savingEdit && setEditingFile(null)}
        >
          <div
    ...
    <div className="p-4 sm:p-5 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-slate-400 mt-1">
          ...
          {new Date(file.created_at).toLocaleString('es-VE', {
            day: 'numeric',
            month: 'short',
            year: 'numeric',
            hour: '2-digit',
        ...
        <div className="p-5 space-y-4">
          {targetDoctor && (
            <div className="bg-teal-50 border border-teal-200 rounded-lg px-3 py-2 text-xs text-teal-800 flex items-center gap-2">
              ...
              <span>
                Para:{' '}
                <b>{`${targetDoctor.professional_title || ''} ${targetDoctor.full_name}`.trim()}</b>
              </span>
            </div>
          )}
          ...
          >
            {submitting ? (
              ...
              <>
                <MessageSquare className="w-4 h-4" /> Enviar solo comentario
              </>
            )}
          </button>
