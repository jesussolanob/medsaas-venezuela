      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-xs sm:text-sm mt-1">
            Configura descuentos por suscripción multi-mes
          </p>
        </div>
        <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              {formOriginalPrice &&
                formPromoPrice &&
                parseFloat(formPromoPrice) < parseFloat(formOriginalPrice) && (
                  <p className="text-[10px] text-emerald-600 mt-0.5 font-semibold">
                    {discount(parseFloat(formOriginalPrice), parseFloat(formPromoPrice))}% de
                    descuento
                  </p>
                )}
            </div>
          ...
          {formPlanKey && formPromoPrice && formOriginalPrice && (
            <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-4 border border-teal-200">
              ...
              <p className="text-sm text-slate-700">
                <span className="font-bold">{PLAN_NAMES[formPlanKey] || formPlanKey}</span> x{' '}
                {formMonths} meses:{' '}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">
            Crea tu primera promoción para atraer más médicos
          </p>
        </div>
      ) : (
        <div className="grid gap-4">
          {promotions.map((promo) => {
            const planName =
              PLAN_NAMES[promo.plan_key] || promo.plan_configs?.name || promo.plan_key;
