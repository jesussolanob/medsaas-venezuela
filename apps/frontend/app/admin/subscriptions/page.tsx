    ...
    <>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={load}
            className="p-2 text-slate-500 hover:text-slate-800 shrink-0"
            title="Recargar"
          >
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400 text-sm">
            Sin doctores que coincidan con el filtro.
          </div>
        ) : (
          <>
            {/* Desktop ≥ md: tabla */}
                    ...
                    return (
                      <tr key={d.doctor_id} className="hover:bg-slate-50">
                        <td className="px-4 py-3">
                          ...
                          {d.specialty && (
                            <div className="text-xs text-teal-600 mt-0.5">{d.specialty}</div>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <span
                        ...
                        <td className="px-4 py-3 text-right">
                          <span
                            className={`font-bold ${d.days_remaining === 0 ? 'text-red-600' : d.days_remaining < 7 ? 'text-orange-600' : 'text-slate-700'}`}
                          >
                            {d.days_remaining}
                          </span>
                        ...
                        <td className="px-4 py-3 text-slate-600 text-xs">
                          {d.current_period_end
                            ? new Date(d.current_period_end).toLocaleDateString('es-VE', {
                                day: '2-digit',
                                month: 'short',
                                year: 'numeric',
                              })
                ...
                return (
                  <div key={d.doctor_id} className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${badge.cls}`}
                      >
                        {badge.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs bg-slate-50 rounded-lg p-2">
                      <div>
                        <div className="text-slate-400 uppercase font-semibold tracking-wider text-[10px]">
                          Días
                        </div>
                        <div
                          className={`text-base font-bold ${d.days_remaining === 0 ? 'text-red-600' : d.days_remaining < 7 ? 'text-orange-600' : 'text-slate-700'}`}
                        >
                          {d.days_remaining}
                        </div>
                      </div>
    ...
    <>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400 text-sm">
            Sin comprobantes en este estado.
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {payments.map((p) => (
              <div
                key={p.id}
                className="p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-3 hover:bg-slate-50"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                  ...
                  {p.rejection_reason && (
                    <div className="text-xs text-red-600 mt-1">Rechazado: {p.rejection_reason}</div>
                  )}
                </div>
...
function ConfigTab() {
  ...
  async function createPromotion() {
    if (newPromo.promo_price_usd >= newPromo.original_price_usd) {
      showToast({ type: 'error', message: 'El precio promocional debe ser menor al original' });
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="grid lg:grid-cols-2 gap-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-slate-400 mt-1">
            Aplicable inmediatamente a nuevas compras. No retroactivo.
          </p>
        </div>
        <div>
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
            Duración del trial Beta (días)
          </label>
          <input
            type="number"
            min={0}
          ...
          />
          <p className="text-xs text-slate-400 mt-1">
            Se aplica solo a nuevos registros. {settings.beta_duration_days} días ≈{' '}
            {Math.round(settings.beta_duration_days / 30)} meses.
          </p>
        </div>
        <div>
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
          ...
          <input
            ...
            onBlur={(e) =>
              save({
                expiration_warning_days: e.target.value
                  .split(',')
                  .map((s) => Number(s.trim()))
                  .filter(Boolean),
              })
          ...
          />
          <p className="text-xs text-slate-400 mt-1">
            Coma-separados. Ej: "7,3,1" notifica 7, 3 y 1 día antes.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <p className="text-xs text-slate-500">
          Estos son los métodos que el doctor verá al pagar su suscripción.
        </p>
        {[
          {
            key: 'pago_movil',
                    ...
                    <input
                      ...
                      onBlur={(e) => {
                        const next = {
                          ...
                          [m.key]: { ...config, [fkey]: e.target.value },
                        };
      ...
      <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <p className="text-xs text-slate-500">
          ...
          pre-rellenado. Configura aquí el número (con código país, sin <code>+</code>) y el
          mensaje.
        </p>
      ...
      <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        <p className="text-xs text-slate-500">
          Define paquetes de varios meses con precio promocional. Ej: 3 meses por $75 (en lugar de
          $90).
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-2 items-end p-3 bg-slate-50 rounded-lg">
          <div>
            ...
            <input
              ...
              min={2}
              max={36}
              value={newPromo.duration_months}
              onChange={(e) =>
                setNewPromo((p) => ({ ...p, duration_months: Number(e.target.value) }))
              }
              className="mt-1 w-full px-2 py-1.5 border border-slate-200 rounded-md text-sm"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={0}
              step="0.01"
              value={newPromo.original_price_usd}
              onChange={(e) =>
                setNewPromo((p) => ({ ...p, original_price_usd: Number(e.target.value) }))
              }
              className="mt-1 w-full px-2 py-1.5 border border-slate-200 rounded-md text-sm"
            />
          </div>
          <div>
            ...
            <input
              ...
              min={0}
              step="0.01"
              value={newPromo.promo_price_usd}
              onChange={(e) =>
                setNewPromo((p) => ({ ...p, promo_price_usd: Number(e.target.value) }))
              }
              className="mt-1 w-full px-2 py-1.5 border border-slate-200 rounded-md text-sm"
            />
          </div>
          ...
          <button
            ...
            disabled={saving}
            className="px-3 py-2 bg-teal-500 text-white rounded-md text-sm font-bold hover:bg-teal-600 disabled:opacity-50"
          >
        ...
        {promotions.length === 0 ? (
          <p className="text-xs text-slate-400 italic text-center py-4">
            Sin promociones configuradas. Solo plan mensual estará disponible.
          </p>
        ) : (
          <div className="space-y-2">
            {promotions.map((p) => {
              const discount =
                p.original_price_usd > 0
                  ? Math.round(
                      ((p.original_price_usd - p.promo_price_usd) / p.original_price_usd) * 100,
                    )
                  : 0;
              return (
                <div
                  key={p.id}
                  className={`flex items-center gap-3 p-3 rounded-lg border ${p.is_active ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200 bg-slate-50'}`}
                >
                  <div className="flex-1">
                    <div className="font-semibold text-slate-800">
                      {p.label || `${p.duration_months} meses`}
                    </div>
                    <div className="text-xs text-slate-500">
                      {p.duration_months} meses ·{' '}
                      <span className="line-through">${p.original_price_usd}</span> →{' '}
                      <strong className="text-emerald-600">${p.promo_price_usd}</strong>
                      <span className="ml-2 text-emerald-600 font-bold">−{discount}%</span>
                    </div>
                  </div>
                  <button
                    onClick={() => togglePromo(p)}
                    className={`px-3 py-1 text-xs font-semibold rounded-md ${p.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}
                  >
