...
const DEFAULT_BILLING: BillingConfig = {
  ...
  rif: 'J-50000000-0',
  domicilio_fiscal:
    'Av. Francisco de Miranda, Torre Delta, Piso 5, Of. 5-A, Urb. El Rosal, Caracas, Miranda, Zona Postal 1060',
...
function buildFiscalInvoice(invoice: InvoiceData, billing: BillingConfig): string {
  ...
  const issueDateObj = invoice.issued_at
    ? new Date(invoice.issued_at)
    : new Date(invoice.created_at);
  const issueDate = issueDateObj.toLocaleDateString('es-VE');
  const issueTime = issueDateObj.toLocaleTimeString('es-VE', {
    hour: '2-digit',
    minute: '2-digit',
  });
  ...
  // Company brand — left
  text(L, 'Delta Salud', ML + 10, 30, 22, true, TEAL_RGB);
  ...
  const fx = fbX + 12;
  let fy = fbY + 34;
  text(L, `No. ${esc(invoice.invoice_number)}`, fx, fy, 8, true, BLACK_RGB);
  fy += 13;
  text(L, `Fecha: ${issueDate}  ${issueTime}`, fx, fy, 7, false, BLACK_RGB);
  fy += 11;
  text(L, `N Control: ${esc(billing.control_number)}`, fx, fy, 7, true, BLACK_RGB);
  fy += 11;
  ...
  // ─────────────────────────────────────────────────────
  const rowY = tblY + 28;
  const description = invoice.description || 'Suscripcion Plan Professional - Delta Salud';
  ...
  taxLine(L, 'Base Exenta:', 'Bs. 0,00', txL, txR, ty);
  ty += 16;
  taxLine(L, 'Base Imponible:', `Bs. ${fmtBs(bsBase)}`, txL, txR, ty);
  ty += 16;
  taxLine(L, `IVA ${billing.iva_percent.toFixed(0)}%:`, `Bs. ${fmtBs(bsIVA)}`, txL, txR, ty);
  ty += 16;
  ...
  taxLine(L, 'Subtotal:', `Bs. ${fmtBs(bsSubtotal)}`, txL, txR, ty, true);
  ty += 18;
  taxLine(L, `IGTF ${billing.igtf_percent.toFixed(0)}%:`, `Bs. ${fmtBs(bsIGTF)}`, txL, txR, ty);
  ty += 20;
  ...
  if (billing.bcv_rate) {
    ...
    text(
      L,
      `Tasa BCV: ${billing.bcv_rate.toFixed(4)} Bs/$`,
      taxBoxX + 10,
      ty + 48,
      7,
      false,
  ...
  text(
    L,
    `Moneda: ${billing.bcv_rate ? 'Bs (ref. BCV)' : 'USD'}`,
    ML + 10,
    payY + 24,
    7,
    false,
  ...
  text(L, 'Delta Salud', ML + 10, PH - 38, 7, true, GRAY_RGB);
  text(
    L,
    'Sistema para Medicos Especialistas | deltamedical.ve',
    ML + 10,
    PH - 26,
    6,
    false,
