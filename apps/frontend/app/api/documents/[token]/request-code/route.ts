 ...
 *
 * Thin-proxy PÚBLICO → NestJS POST /api/documents/:token/request-code
 * SIN headers de auth — endpoint público.
 *
 * Genera y reenvía un nuevo código al email del paciente.
 * Respuesta exitosa: { expiresAt }
 */
...
export async function POST(
  ...
  try {
    backendRes = await fetch(`${BACKEND_URL}/api/documents/${token}/request-code`, {
      method: 'POST',
