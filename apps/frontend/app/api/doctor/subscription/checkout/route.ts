 ...
 *
 * FASE 5/6: pendiente backend — payment receipts (Supabase Storage) and
 * subscription_payments table not yet implemented in NestJS backend.
 * Using createAdminClient locally until the billing/storage module is migrated.
 */
...
export async function POST(req: NextRequest) {
  ...
  // FASE 5/6: pendiente backend — Supabase admin used for storage upload + subscription_payments insert
  ...
  const promotion_id = formData.get('promotion_id')
    ? String(formData.get('promotion_id'))
    : null;
  ...
  if (!reference_number) {
    return NextResponse.json(
      { error: 'Referencia/nro de comprobante requerido' },
      { status: 400 },
    );
  }
  if (duration_months < 1 || duration_months > 36) {
    return NextResponse.json(
      { error: 'Duración debe estar entre 1 y 36 meses' },
      { status: 400 },
    );
  }
  ...
  if (receipt && typeof receipt === 'object' && 'arrayBuffer' in receipt) {
    ...
    if (!['jpg', 'jpeg', 'png', 'pdf', 'webp', 'heic'].includes(ext)) {
      return NextResponse.json(
        { error: 'Formato no soportado (jpg/png/pdf/webp)' },
        { status: 400 },
      );
    }
    const path = `${guard.user.id}/${Date.now()}.${ext}`;
    const buf = Buffer.from(await receipt.arrayBuffer());
    const { error: upErr } = await admin.storage
      .from('payment-receipts')
      .upload(path, buf, {
        contentType: receipt.type || `application/${ext}`,
        upsert: false,
      });
    if (upErr) {
