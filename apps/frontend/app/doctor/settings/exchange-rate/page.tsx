    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Elige qué tasa usar para convertir tus precios USD/EUR a bolívares en citas, facturas y
          reportes.
        </p>
      </div>
      ...
      <div className="bg-gradient-to-br from-cyan-50 to-teal-50 border border-cyan-200 rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg p-4 border border-slate-200">
            ...
            <p className="text-2xl font-bold text-slate-900 mt-1">
              {eurRateBcv
                ? eurRateBcv.toLocaleString('es-VE', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 4,
                  })
                : '—'}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
        ...
        <label
          className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${mode === 'usd_bcv' ? 'border-teal-500 bg-teal-50' : 'border-slate-200 hover:bg-slate-50'}`}
        >
          <input
            type="radio"
            name="mode"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Se actualiza automáticamente desde el Banco Central de Venezuela. Tus precios en USD
              se convierten a Bs usando la tasa oficial.
            </p>
          </div>
        </label>

        <label
          className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${mode === 'eur_bcv' ? 'border-teal-500 bg-teal-50' : 'border-slate-200 hover:bg-slate-50'}`}
        >
          <input
            type="radio"
            name="mode"
        ...
        <label
          className={`flex items-start gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${mode === 'custom' ? 'border-teal-500 bg-teal-50' : 'border-slate-200 hover:bg-slate-50'}`}
        >
          <input
            type="radio"
            name="mode"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Define tú mismo la tasa (útil si usas tasa paralela o una tasa específica de tu
              clínica). No se actualiza automáticamente.
            </p>
            {mode === 'custom' && (
            ...
            {previewLoading ? (
              ...
              <>
                <p className="text-3xl font-bold text-slate-900 mt-1">
                  {previewRate.toLocaleString('es-VE', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 4,
                  })}{' '}
                  Bs
