...
export default function CRMPage() {
  ...
  const handleSendMessage = async () => {
    ...
    setLeads((prev) =>
      prev.map((l) =>
        l.id === selectedLead.id ? { ...l, last_activity: new Date().toISOString() } : l,
      ),
    );
  };
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Gestiona tus leads con Kanban — arrastra para cambiar etapa
              </p>
            </div>
            <button
            ...
            <form onSubmit={handleAddLead} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewLeadData((p) => ({ ...p, channel: e.target.value as LeadChannel }))
                  }
                  className="input-field"
                >
                  {(Object.entries(CHANNEL_LABELS) as [LeadChannel, string][]).map(
                    ([ch, label]) => (
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full lg:w-96 bg-white shadow-2xl flex flex-col z-50">
            ...
            <div className="px-4 py-3 border-b border-slate-200 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-slate-900">
                  {getTimeAgo(selectedLead.last_activity || selectedLead.created_at)}
                </span>
              </div>
            </div>
                  ...
                  >
                    <div
                      className={`max-w-xs px-4 py-2 rounded-lg text-sm ${
                        msg.sender === 'doctor' ? 'g-bg text-white' : 'bg-slate-100 text-slate-900'
                      }`}
                    >
                      {msg.content}
                      <p
                        className={`text-xs mt-1 ${msg.sender === 'doctor' ? 'text-white/70' : 'text-slate-500'}`}
                      >
            ...
            <div className="border-t border-slate-200 p-4 space-y-3">
              ...
              <button
                onClick={() =>
                  handleContactViaChannel(selectedLead, messageInput || selectedLead.message)
                }
