          ...
          {filtered.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* ADR-025 rev.2: price_usd es el precio de TODO el paquete. Ya no
                    se multiplica ni se muestra el "× N =": lo que el especialista
                    escribió es lo que se cobra. */}
                <div className="mb-3">
                  <div className="flex items-baseline gap-1">
                    ...
                    {item.sessions_count > 1 && (
                      <span className="text-xs text-slate-500 ml-1">
                        por el paquete de {item.sessions_count}
                      </span>
                    )}
                  </div>
                  {bcvRate && (
                    <p className="text-xs text-slate-400 mt-0.5">{toBs(item.price_usd)}</p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500 mb-4">
                  {item.sessions_count > 1 && (
                    <div className="flex items-center gap-1">
                      ...
                      {item.sessions_count} sesiones incluidas
                    </div>
                  )}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                {/* ADR-025 rev.2: se carga el precio de TODO el paquete. La etiqueta
                    decía "unitario" y era la fuente de la confusión. */}
                <label className="block text-xs font-medium text-slate-600 mb-1.5">
                  {(parseInt(sessionsCount) || 1) > 1
                    ? `Precio ${currencyCode} del paquete completo`
                    : `Precio ${currencyCode}`}{' '}
              ...
              {type === 'plan' && (
                <div>
                  ...
                  {/* Antes acá se mostraba "Total del paquete = precio × sesiones".
                      Ya no hay multiplicación: el precio cargado ES el del paquete.
                      Se deja el desglose informativo por sesión, que es lo que el
                      paciente pregunta, pero NO se cobra a partir de él. */}
                  {(() => {
                      ...
                      return (
                        <div className="mt-2 px-3 py-2 rounded-lg bg-teal-50 border border-teal-100">
                          <p className="text-xs font-semibold text-teal-700">
                            El paciente paga {format(p)} por las {s} consultas{' '}
                            <span className="font-normal text-teal-600">
                              (equivale a {format(p / s)} {currencyCode} cada una)
                            </span>
                          </p>
                        </div>
                      );
