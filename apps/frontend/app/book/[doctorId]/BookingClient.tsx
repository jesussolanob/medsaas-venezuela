...
const fi =
  'w-full px-4 py-3 text-sm border border-slate-200 rounded-xl outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500/10 bg-white transition-colors';
  ...
  return (
    <div
      className={`rounded-2xl overflow-hidden transition-all duration-300 ${
        isOpen
          ? 'shadow-lg shadow-cyan-500/10 bg-white ring-1 ring-cyan-400/50'
          : isPast
    ...
    >
      ...
      {isOpen && <div className="px-5 pb-5 pt-1 animate-in fade-in duration-200">{children}</div>}
    </div>
  );
}
...
export default function BookingClient({
  ...
  async function handleSubmit() {
    ...
    if (!appointmentMode) {
      setError('Selecciona modalidad de consulta');
      submittingRef.current = false;
      return;
    }
    if (!usingPackage && !useInsurance && !selectedPaymentMethod) {
      setError('Selecciona un método de pago');
      submittingRef.current = false;
      return;
    }
    if (!usingPackage && useInsurance && !selectedInsurance) {
      setError('Selecciona tu seguro');
      submittingRef.current = false;
      return;
    }
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 p-8 max-w-sm w-full text-center">
          ...
          >
            ...
            <p className="text-xs text-slate-600">
              <span className="font-semibold">📅 Fecha:</span> {selectedSlot?.label} a las{' '}
            ...
            <p className="text-xs text-slate-600">
              <span className="font-semibold">
                {appointmentMode === 'online' ? '💻' : '🏥'} Modalidad:
              </span>{' '}
            ...
            {appointmentMode === 'presencial' && !selectedOffice && doctor.office_address && (
              <p className="text-xs text-slate-600">
                <span className="font-semibold">📍 Dirección:</span> {doctor.office_address}
              </p>
            )}
            {appointmentMode === 'online' && bookedMeetLink && (
              <div className="mt-1 pt-2 border-t border-slate-100">
                <p className="text-xs font-semibold text-blue-700 mb-1">Link de videoconsulta:</p>
                <a
                  href={bookedMeetLink}
                  target="_blank"
            ...
            {appointmentMode === 'online' && !bookedMeetLink && (
              <p className="text-[10px] text-slate-500 italic">
                Recibirás el link de la videoconsulta por correo o WhatsApp.
              </p>
            )}
          </div>
              ...
              href={(() => {
                ...
                const details = encodeURIComponent(
                  `Plan: ${selectedPlan?.name || 'Consulta'}\n` +
                    ...
                    (selectedOffice
                      ? `Dirección: ${selectedOffice.address}, ${selectedOffice.city}`
                      : ''),
                );
            ...
            activePackage.total_sessions - activePackage.used_sessions > 0 && (
              <div className="bg-violet-50 border border-violet-200 rounded-xl px-4 py-3 mb-4 text-left">
                <p className="text-xs text-violet-700 font-semibold">
                  Te quedan {activePackage.total_sessions - activePackage.used_sessions} cita
                  {activePackage.total_sessions - activePackage.used_sessions !== 1 ? 's' : ''} en
                  tu paquete
                </p>
              </div>
            )}
        ...
        <div className="max-w-lg mx-auto px-4 py-5 space-y-3">
                ...
                <button
                  ...
                  className={`w-full text-left rounded-xl p-4 transition-all border-2 ${
                    usingPackage
                      ...
                      : 'border-violet-300 bg-violet-50/50 hover:border-violet-500'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-violet-600 mt-0.5">
                        Te quedan {activePackage.total_sessions - activePackage.used_sessions} cita
                        {activePackage.total_sessions - activePackage.used_sessions !== 1
                          ? 's'
                          : ''}{' '}
                        — ya pagadas
                      </p>
                ...
                return (
                  <button
                    ...
                    className={`relative w-full text-left rounded-xl p-4 transition-all ${
                      isSelected
                        ? 'border-2 border-cyan-500 bg-cyan-50/50 shadow-md shadow-cyan-100'
                        : isMiddle
                          ? 'border-2 border-cyan-300 bg-white hover:border-cyan-400'
                          : 'border-2 border-slate-200 bg-white hover:border-cyan-300'
                    }`}
                  >
                    {isMiddle && !isSelected && (
                      <span
                        className="absolute -top-2.5 left-3 text-[10px] font-bold bg-white px-2 py-0.5 rounded-full border border-cyan-200"
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <span className="text-xs font-semibold text-slate-600">
                  {weekDates.length > 0 &&
                    `${new Date(weekDates[0] + 'T12:00:00').toLocaleDateString('es-VE', { day: 'numeric', month: 'short' })} — ${new Date(weekDates[weekDates.length - 1] + 'T12:00:00').toLocaleDateString('es-VE', { day: 'numeric', month: 'short' })}`}
                </span>
                <button
              ...
              <AccordionSection
                ...
                summary={
                  onlyInPerson
                    ...
                    : onlyOnline
                      ? 'Videoconsulta (único modo disponible)'
                      : appointmentMode === 'online'
                        ? 'Videoconsulta (online)'
                  ...
                  {(onlyInPerson || onlyOnline) && (
                    <div className="bg-teal-50 border border-teal-200 rounded-xl p-3">
                      <p className="text-xs text-teal-700">
                        {onlyInPerson
                          ? 'Este consultorio solo ofrece atención presencial.'
                          : 'Este consultorio solo ofrece atención online (videollamada).'}
                      </p>
                    </div>
                  )}

                  <div
                    className={`grid gap-3 ${!onlyInPerson && !onlyOnline ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}
                  >
                    {allowsInPerson && (
                      <button
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            ...
                            style={{
                              background:
                                appointmentMode === 'presencial' ? '#06B6D410' : '#f1f5f9',
                            }}
                          >
                            <MapPin
                              className="w-5 h-5"
                              style={{
                                color:
                                  appointmentMode === 'presencial' ? BRAND.turquoise : '#94a3b8',
                              }}
                            />
                          </div>
                          <div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0"
                            style={{
                              background: appointmentMode === 'online' ? '#3b82f610' : '#f1f5f9',
                            }}
                          >
                            <Video
                              className="w-5 h-5"
                              style={{
                                color: appointmentMode === 'online' ? '#3b82f6' : '#94a3b8',
                              }}
                            />
                          </div>
                          <div>
                    ...
                    <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {selectedOffice.phone && (
                            <p className="text-xs text-cyan-600 mt-0.5">{selectedOffice.phone}</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  {appointmentMode === 'presencial' && !selectedOffice && selectedDate && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
                      <p className="text-xs text-amber-700">
                        {doctor.office_address ||
                          'Consulta con el médico la dirección del consultorio para este día.'}
                      </p>
                    </div>
                  )}
                </div>
              ...
              {authUser ? (
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                    ...
                    <div className="space-y-1 ml-12">
                      <p className="text-xs text-slate-600">
                        ...
                        {form.full_name || authUser.user_metadata?.full_name}
                      </p>
                ...
                <div className="space-y-3">
                  <p className="text-sm text-slate-600">
                    Puedes iniciar sesión para acceder a tus paquetes prepagados, o continuar como
                    invitado.
                  </p>
                ...
                <form onSubmit={handleAuthLogin} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="w-full text-white py-3 rounded-xl text-sm font-bold transition-all hover:opacity-90 disabled:opacity-60"
                    style={{ background: BRAND.turquoise }}
                  >
                  ...
                  <details className="border border-slate-200 rounded-xl bg-slate-50">
                    ...
                    <div className="p-3 space-y-2.5 bg-white border-t border-slate-200 rounded-b-xl">
                      <p className="text-[11px] text-slate-500">
                        Estos datos ayudan a tu médico. Puedes saltarlos y completarlos más tarde
                        desde tu perfil.
                      </p>
            ...
            <AccordionSection
              ...
              summary={
                useInsurance
                  ...
                  : selectedPaymentMethod
                    ? PAYMENT_LABELS[selectedPaymentMethod as PaymentMethod]
                    : undefined
              }
              completed={
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {selectedPaymentMethod &&
                    requiresReceipt(selectedPaymentMethod as PaymentMethod) && (
                      <div
                        className="border border-dashed border-slate-300 rounded-xl p-4 space-y-2"
                      ...
                      >
                        ...
                        <label
                          ...
                          style={{ borderColor: `${BRAND.coral}40` }}
                        >
                          <input
                            type="file"
                    ...
                    !requiresReceipt(selectedPaymentMethod as PaymentMethod) && (
                      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                        <p className="text-xs text-emerald-700">
                          <span className="font-semibold">Nota:</span> Pagarás el día de la consulta
                          ({PAYMENT_LABELS[selectedPaymentMethod as PaymentMethod]})
                        </p>
                      </div>
                    )}
          ...
          >
            <div className="space-y-4">
              {/* Summary card */}
              <div
                className={`rounded-xl p-4 space-y-3 text-sm ${usingPackage ? 'bg-violet-50 border border-violet-200' : 'bg-cyan-50/50 border border-cyan-200'}`}
              >
                <div className="flex items-center justify-between">
                  <span
                    className={`font-bold ${usingPackage ? 'text-violet-700' : ''}`}
                    style={!usingPackage ? { color: BRAND.ink } : undefined}
                  >
                    {selectedPlan?.name}
                  </span>
                  {usingPackage && activePackage ? (
                ...
                <div
                  className={`flex items-center gap-2 text-xs ${usingPackage ? 'text-violet-600' : 'text-cyan-700'}`}
                >
                  <Calendar className="w-3.5 h-3.5" />
                  <span>
                    {selectedSlot?.label} a las {selectedSlot?.time}
                  </span>
                </div>
                <div
                  className={`flex items-center gap-2 text-xs font-semibold px-2.5 py-1.5 rounded-lg w-fit ${
                    appointmentMode === 'online'
                ...
                {usingPackage && (
                  <p className="text-xs text-violet-600 font-medium">
                    Ya pagado con tu paquete activo
                  </p>
                )}
              </div>
              ...
              <div className="rounded-xl p-3.5 space-y-1" style={{ background: BRAND.bone }}>
                ...
                <p className="text-xs text-slate-600">
                  <span className="font-semibold">Email:</span> {authUser?.email || form.email}
                </p>
                {(form.phone || authUser?.user_metadata?.phone) && (
                  <p className="text-xs text-slate-600">
                    <span className="font-semibold">Teléfono:</span>{' '}
