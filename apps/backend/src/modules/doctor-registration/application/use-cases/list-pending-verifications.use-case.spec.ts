...
describe('ListPendingVerificationsUseCase', () => {
  ...
  it('delegates to repo with given status', async () => {
    ...
    const results = await useCase.execute({ status: 'pending' });
  ...
  it('returns empty array when no doctors match status', async () => {
    ...
    const results = await useCase.execute({ status: 'verified' });
  ...
  it('propagates repo errors', async () => {
    ...
    await expect(useCase.execute({ status: 'pending' })).rejects.toThrow('DB error');
  });
});
