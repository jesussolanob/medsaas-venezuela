import { RejectConsultationPaymentUseCase } from './reject-consultation-payment.use-case';
...
describe('RejectConsultationPaymentUseCase', () => {
  let useCase: RejectConsultationPaymentUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new RejectConsultationPaymentUseCase(mockRepo);
  });

  it('transitions pending → rejected and persists', async () => {
    const pending = makePayment({ status: 'pending' });
    const rejected = makePayment({ status: 'rejected' });
    mockRepo.findByIdForDoctor.mockResolvedValue(pending);
    mockRepo.save.mockResolvedValue(rejected);
