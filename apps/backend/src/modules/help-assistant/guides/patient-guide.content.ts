...
> documentos compartidos) y otras estan EN PREPARACION (mensajes con el medico, seguimiento
...
3. **Recibir y descargar documentos medicos** (informes de consulta, recetas, historia clinica)
   que tu medico te comparte de forma segura por enlace y codigo.
...
medicamentos) se guardan cifrados. Solo tu medico y tu pueden acceder a ellos.

---

## 2. Como reservar una cita (booking publico del medico)

Cada medico tiene un enlace publico de reservas con la forma /book/ seguido de su identificador
(por ejemplo /book/ID-DEL-MEDICO). Tu medico te comparte ese enlace por WhatsApp, correo o redes.
Al abrirlo veras una pagina con el nombre del medico, su especialidad, su foto y su ciudad, y un
...
Veras las tarjetas de los servicios que ofrece el medico. Cada tarjeta muestra:
...
Si ya tienes un **paquete prepagado activo** con ese medico (sesiones que ya pagaste antes),
...
Depende de como tenga configurada su atencion el medico:
...
- Si el medico **solo atiende por videoconsulta**, veras un aviso "Solo disponible por
  videoconsulta" y un boton "Continuar".
- Si el medico **no tiene consultorios ni videoconsulta configurada**, aparece "Sin disponibilidad
...
acepta el medico. Las opciones posibles son:
- Pago Movil
- Transferencia
- Zelle
- Binance
...
medico y debes contactarlo directamente:
- **"Reservas en linea no disponibles"**: el plan del medico no tiene habilitada la reserva en
  linea. Esto ocurre tipicamente cuando el medico esta en el plan gratuito (Delta Free), que no
  incluye la funcion de reservas. No es un error tuyo.
- **"Reservas no disponibles"**: el medico todavia no configuro servicios o tarifas para agendar.
- **"Medico no encontrado"**: el enlace no es valido o el medico ya no esta disponible.
...
- La seccion **"Paquetes activos"** (si tienes paquetes prepagados): cada paquete muestra el
  ...
  del doctor y un boton **"Agendar siguiente cita"** que te lleva al booking de ese medico para
  usar una sesion.
...
medico; el o su personal hacen el cambio desde su agenda. Si quieres una nueva cita, usa el boton
"Agendar otra cita".
...
Mientras tanto, la via segura por la que tu medico te entrega tus documentos medicos es el
...
compartido con tu medico (tareas, archivos y mensajes en un solo lugar) estara disponible
proximamente. Mientras tanto, la recomendacion es consultar directamente con tu medico.
...
corregir tu nombre, cedula u otros datos clinicos de inmediato, comunicaselo a tu medico.
...
- **Mensajes** (/patient/messages): el chat con el medico esta fuera de esta version; te redirige
  al inicio del portal.

---

## 4. Como abrir un documento medico que te comparte tu medico

Cuando tu medico te comparte documentos (informe de consulta, recetas o historia clinica), recibes
...
5. Si todo es correcto, veras **"Acceso verificado"** con: el nombre del medico, tu nombre
...
- Si despues de solicitar un codigo nuevo sigue sin funcionar, contacta a tu medico para que te
...
Abre el enlace de reservas de tu medico (/book/ seguido de su identificador) y sigue los pasos del
...
**Por que no puedo reservar en linea con cierto medico?**
Si ves "Reservas en linea no disponibles", el plan de ese medico (normalmente el plan gratuito
Delta Free) no incluye la reserva en linea. Si ves "Reservas no disponibles", aun no configuro sus
servicios. En ambos casos, contacta al medico directamente para coordinar la cita.
...
medico (o a su personal) para que cancele o cambie la fecha desde su agenda. Si lo que quieres es
...
**Como veo el documento que me envio mi medico?**
...
nuevo reactiva el acceso. Como ultimo recurso, pidele a tu medico que vuelva a compartir el
documento.
...
cedula mas un codigo de un solo uso para descargarse. Solo tu y tu medico acceden a tu informacion.
