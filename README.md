...
de historial — en prod con Gemini, gating por plan).

El **admin super_admin** es 100% configurable: planes/precios/features parametrizables,
bloqueo/desbloqueo de acceso de doctor (ban independiente), especialidades (crear/editar/activar),
app-settings genéricos, **editor de plantillas de email** (9 plantillas, preview sandbox, variables visuales),
diagnóstico de envíos de correo (`email_send_log`, cero PII).
...
| Observabilidad | **Sentry** (gated por env) + `GlobalExceptionFilter` + `email_send_log` (diagnóstico)       |
| IA             | **Google Gemini** (transcripción + texto: mejorar/resumir/historial — en prod, gating plan) |
