    ...
    <>
      ...
      <div className="max-w-4xl space-y-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <button key={s.filter} onClick={() => setTimeFilter(timeFilter === s.filter ? 'all' : s.filter)}
              className={`border rounded-xl p-4 text-center transition-all hover:shadow-sm ${s.bg} ${timeFilter === s.filter ? 'ring-2 ring-teal-400 ring-offset-1' : 'border-slate-200'}`}>
        ...
        <div className="flex gap-3">
          ...
          <select value={timeFilter} onChange={e => setTimeFilter(e.target.value as TimeFilter)} className="border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-teal-400 text-slate-600 bg-white">
            ...
            filtered.map((c, i) => {
              ...
              return (
                <button key={c.id} onClick={() => openConsultation(c)}
                  className={`w-full flex items-start gap-4 px-5 py-4 text-left hover:bg-slate-50 transition-colors ${i < filtered.length - 1 ? 'border-b border-slate-100' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      ...
                      <span className="text-[10px] font-mono text-slate-400">{c.consultation_code}</span>
                      {isToday && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">Hoy</span>}
                      {!isToday && isUpcoming && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">Próxima</span>}
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                      ...
                      {c.chief_complaint && <><span className="text-slate-200">·</span><span className="italic truncate max-w-[200px]">{c.chief_complaint}</span></>}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {hasReport && <span className="text-[10px] font-semibold text-violet-600 bg-violet-50 px-2 py-0.5 rounded-full">Con informe</span>}
