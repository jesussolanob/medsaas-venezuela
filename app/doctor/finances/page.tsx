      ...
      <div className="max-w-6xl space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500 mt-1">Gestiona ingresos, gastos, seguros y estado de pagos</p>
          </div>
          <button
            onClick={() => {
              const getBCVRate = async () => {
            ...
            className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-semibold hover:bg-slate-200 transition-colors shrink-0"
          >
            <TrendingUp className="w-3 h-3 hidden sm:inline" />
            <span className="whitespace-nowrap">BCV · Bs.S {bcvLoading ? '...' : (bcvRate?.toLocaleString('es-VE', { minimumFractionDigits: 2 }) ?? '36.50')}</span>
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1 flex-wrap overflow-x-auto">
          {([['overview', 'Resumen'], ['income', 'Ingresos'], ['insurance', 'Seguros'], ['expenses', 'Gastos'], ['pl', 'Balance P&L'], ['payments', 'Estado Pagos']] as [Tab | 'insurance', string][]).map(([t, label]) => (
            <button key={t} onClick={() => setTab(t as Tab)} className={`px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap shrink-0 ${tab === t ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{label}</button>
          ))}
        </div>
