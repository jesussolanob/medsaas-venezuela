...
import {
  ...
  User, Sparkles, Search, ArrowRight
} from 'lucide-react'

type CommandItem = { name: string; href: string; icon: any; description: string }

const commands: CommandItem[] = [
  { name: 'Inicio', href: '/patient', icon: LayoutDashboard, description: 'Mi panel principal' },
  { name: 'Mis citas', href: '/patient/appointments', icon: Calendar, description: 'Historial de citas médicas' },
  { name: 'Mis informes', href: '/patient/reports', icon: FileText, description: 'Informes y resultados' },
  { name: 'Recetas', href: '/patient/prescriptions', icon: Pill, description: 'Mis recetas médicas' },
  { name: 'Mensajes', href: '/patient/messages', icon: MessageCircle, description: 'Chat con mi doctor' },
  { name: 'Mi perfil', href: '/patient/profile', icon: User, description: 'Datos personales y médicos' },
]
...
export default function SearchCommandPalette() {
  ...
  const handleOpen = useCallback(() => { setOpen(true); setQuery(''); setSelectedIndex(0); setTimeout(() => inputRef.current?.focus(), 50) }, [])
  ...
  return (
    <>
      <button onClick={handleOpen} className="hidden md:flex items-center gap-2 px-4 py-2 rounded-full transition-all" style={{ background: '#F4F6F8', minWidth: 200, border: '1px solid transparent' }}>
          ...
          <div className="relative flex justify-center" style={{ paddingTop: '15vh' }}>
            <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl overflow-hidden" style={{ background: '#fff', border: '1px solid #E8ECF0', boxShadow: '0 25px 60px rgba(15,26,42,0.15)', animation: 'cmdIn .15s ease' }}>
              ...
              <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: '1px solid #E8ECF0' }}>
                ...
                <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar sección..." className="flex-1 text-sm outline-none" style={{ color: '#0F1A2A', fontFamily: "'Plus Jakarta Sans', sans-serif" }} />
