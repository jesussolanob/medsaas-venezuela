'use client'
import { useState } from 'react'

interface SubscriptionActionButtonsProps {
  subscriptionId: string
  subscriptionStatus: string
  doctorId: string
  onSuccess?: () => void
}

export default function SubscriptionActionButtons({
  subscriptionId,
  subscriptionStatus,
  doctorId,
  onSuccess,
}: SubscriptionActionButtonsProps) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const handleToggleStatus = async () => {
    const action = subscriptionStatus === 'active' ? 'suspend' : 'activate'
    const message =
      subscriptionStatus === 'active'
        ? '¿Estás seguro de que deseas suspender esta suscripción?'
        : '¿Estás seguro de que deseas activar esta suscripción?'

    if (!confirm(message)) return

    try {
      setLoading(true)
      setError('')

      const response = await fetch('/api/admin/toggle-subscription', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subscriptionId, action }),
      })

      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Error actualizando suscripción')
      }

      // Refresh the page to show updated data
      window.location.reload()
      onSuccess?.()
    } catch (err: any) {
      console.error('Error toggling subscription status:', err)
      setError(err.message || 'Error al actualizar la suscripción')
    } finally {
      setLoading(false)
    }
  }

  return (
    <>
      <button
        onClick={handleToggleStatus}
        disabled={loading}
        className={`text-xs font-medium transition-colors disabled:opacity-60 ${
          subscriptionStatus === 'active'
            ? 'text-red-500 hover:text-red-700'
            : 'text-emerald-500 hover:text-emerald-700'
        }`}
      >
        {loading ? 'Actualizando...' : subscriptionStatus === 'active' ? 'Suspender' : 'Activar'}
      </button>
      {error && <div className="text-xs text-red-500 mt-1">{error}</div>}
    </>
  )
}
