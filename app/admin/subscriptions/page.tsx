...
export default async function SubscriptionsPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h2 className="text-xl sm:text-2xl font-semibold text-slate-900">Suscripciones</h2>
        <p className="text-slate-500 text-xs sm:text-sm mt-1">{subscriptions?.length ?? 0} suscripciones en total</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        {['active', 'trial', 'suspended', 'past_due'].map((status) => {
          ...
          return (
            <div key={status} className="bg-white rounded-xl border border-slate-200 p-3 sm:p-4">
              <div className={`w-8 h-8 rounded-lg ${config.bg} flex items-center justify-center mb-2 sm:mb-3`}>
                <config.icon className={`w-4 h-4 ${config.color}`} />
              </div>
              <p className="text-xl sm:text-2xl font-semibold text-slate-900">{count}</p>
        ...
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-slate-500 uppercase tracking-wider hidden lg:table-cell">Precio</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {!subscriptions || subscriptions.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 sm:px-6 py-16 text-center text-slate-400 text-sm">
                  ...
                  <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                        <div className="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-medium text-xs sm:text-sm flex-shrink-0">
                          {sub.profiles?.full_name?.charAt(0) ?? '?'}
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs sm:text-sm font-medium text-slate-900 truncate">{sub.profiles?.full_name}</p>
                          <p className="text-xs text-slate-400 truncate">{sub.profiles?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4">
                    ...
                    <td className="px-4 sm:px-6 py-4 text-xs sm:text-sm text-slate-600 hidden md:table-cell">{vence}</td>
